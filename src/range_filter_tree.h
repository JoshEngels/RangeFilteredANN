...
struct FlatRangeFilterIndex {
  ...
  // TODO: make points a unique_ptr to a PR. Currently stack allocating like a
  // moron.
  std::unique_ptr<PR> points;

  parlay::sequence<index_type>
      indices; // indices of points wrt the original dataset, important for
               // returning results
  ...
  parlay::sequence<index_type>
      sorted_filter_indices; // indices wrt the subset sorted by filter value
  ...
  // the below will have to be modified to be a variant if PR is not a subset PR
  // at the top level additionally, will have to consider how to handle the
  // equivalent case in the real index, perhaps with a variant
  std::pair<std::unique_ptr<FlatRangeFilterIndex<
                T, Point, SubsetPointRange<T, Point>, FilterType>>,
            std::unique_ptr<FlatRangeFilterIndex<
                T, Point, SubsetPointRange<T, Point>, FilterType>>>
      children;

  // FlatRangeFilterIndex();

  /* an interface to support std::move-ing unique ptrs to the point range
   * directly */
  FlatRangeFilterIndex(std::unique_ptr<PR> points,
    ...
    range =
        std::make_pair(sorted_filter_values[0], sorted_filter_values[n - 1]);
    ...
    if (recurse) {
      ...
      std::cout << "Number of leaves with valid point ranges: " << b
                << std::endl;
    }
  }
   ...
   */
  FlatRangeFilterIndex(const PR &points,
                       const parlay::sequence<FilterType> &filter_values,
                       int32_t cutoff = 1000, bool recurse = true) {
    auto unique_points = std::make_unique<PR>(points);
    *this = FlatRangeFilterIndex<T, Point, PR, FilterType>(
        std::move(unique_points), filter_values, cutoff, recurse);
  }

  /* This constructor should be used by the Python layer */
  FlatRangeFilterIndex(py::array_t<T> points,
    ...
    *this = FlatRangeFilterIndex<T, Point, PR, FilterType>(
        point_range, filter_values_seq, cutoff, true);
  }
  ...
  self_query(const Point &query, const std::pair<FilterType, FilterType> &range,
    ...
    //     this->range.first << ", " << this->range.second << ") index range vs.
    //     (" << range.first << ", " << range.second << ")" << std::endl; return
    //     parlay::sequence<pid>();
    // }

    parlay::sequence<pid> frontier = parlay::sequence<pid>(
        knn, std::make_pair(-1, std::numeric_limits<FilterType>::max()));

    // identify the points that are within the filter range
    size_t start = 0;
    size_t end = points->size();

    // if the start of the query range could fall within the index range, find
    // the first point that is >= the start of the query range
    ...
    // if the start and end are the same, there are no points within the query
    // range
    if (start == end) {
    ...
    for (auto i = start; i < end; i++) {
      size_t j =
          sorted_filter_indices[i]; // local index of the point (wrt subset)
  ...
  /* This should do exhaustive search on every subrange contained entirely
   * within the query range, and preprocess leaves. */
  parlay::sequence<pid>
  ...
  void build_children() {
    ...
    std::unique_ptr<SubsetPointRange<T, Point>> points1 = points->make_subset(
        parlay::map(parlay::make_slice(sorted_filter_indices.begin(),
                                       sorted_filter_indices.begin() + n1),
                    [&](auto i) { return indices[i]; }));
    std::unique_ptr<SubsetPointRange<T, Point>> points2 = points->make_subset(
        parlay::map(parlay::make_slice(sorted_filter_indices.begin() + n1,
                                       sorted_filter_indices.end()),
                    [&](auto i) { return indices[i]; }));

    auto filter_values1 = parlay::sequence<FilterType>(
        sorted_filter_values.begin(), sorted_filter_values.begin() + n1);
    auto filter_values2 = parlay::sequence<FilterType>(
        sorted_filter_values.begin() + n1, sorted_filter_values.end());

    auto index1 = std::make_unique<
        FlatRangeFilterIndex<T, Point, SubsetPointRange<T, Point>, FilterType>>(
        std::move(points1), filter_values1, this->cutoff, false);
    auto index2 = std::make_unique<
        FlatRangeFilterIndex<T, Point, SubsetPointRange<T, Point>, FilterType>>(
  ...
  /* This method counts the leaves of the tree and how many of them have a
   * subset point range which points to a point range with the same dimension
   * (and is therefore probably not junk values) */
